...
c     >            'mc_1.rzdat','mc_2.rzdat','mc_3.rzdat'/
        ...
        real*4 prescale,cltime,trackeff,trigeff,cereff,caloeff
...
c        do 
     ...
     &            trigeff,cereff,caloeff,rate_dat
c            if(runnum .EQ. runid) exit
c          if(stat /= 0) then 
c            write(6,*) "Can't find run #",
c     &                  id, " info in runplan.inp.  Stop"
c            stop
c          endif
c        enddo
 	...
 	write(6,*) 'Comp. Ltime = ', cltime
        ...
        write(6,*) 'Cer Eff.   = ', cereff
        write(6,*) 'Calo Eff.   = ', caloeff
...
!!      density correction for gas target
	...
	fract = fract*trackeff*trigeff*cltime*eltime
     &            *cereff*caloeff/prescale*denscor
