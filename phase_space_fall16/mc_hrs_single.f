...
C HBOOK/NTUPLE common block and parameters.
     ...
     >                  'elossi','elossf','eloss0','theta_rec'/
...
!       6: single carbon foil
	...
	if(tarid.GT.7.or.tarid.LT.0)  then
	   tarid = 2
	   write(6,*) 'Invalid input.Use default vertex flag (tarid = 3, H3 gas)'
	endif
...
c	write(filename,'("outfiles/src_",i5,"_",i1,".hist")') runid,tarid
	write(filename,'("outfiles/T2_",i5,"_",i1,".hist")') runid,tarid
...
!	if (.not.rd_real(str_line,rad_len_cm)) stop 'ERROR (RAD_LEN_CM) in setup!'
	...
	if(tarid==2) rad_len_cm = 122.6 !D2
	if(tarid==3) rad_len_cm = 183.6 !H3
...
c	        if(wall==4) s_Al = forward_path
		...
		if(tarid.LE.4) s_Al = s_wall / abs(sin(th_ev)) ! gas->side wall
		if(tarid==5) then
